            ...
            <div className='basis-[155px]'>
                ...
                >
                    <div className={'hidden group-hover:flex relative z-10 bg-[rgba(0,0,0,0.7)] py-1 items-center text-md rounded-lg ' + (isBig ? "px-2 mb-3 gap-2 justify-center" : "mb-[2px] w-full justify-around mx-[2px]")}>
                        <button onClick={toggleIsWatched}><FontAwesomeIcon className={filmState.isWatched ? 'text-green-400' : ""} icon={faEye} /></button>
                        <button onClick={toggleIsLiked}><FontAwesomeIcon className={filmState.isLiked ? 'text-[#FF9111]' : ""} icon={faHeart} /></button>
                ...
                {isRatingOn &&
                    <div>
                        {"★".repeat((filmState.rate + 1) / 2) + ((filmState.rate + 1) % 2 === 1 ? "½" : "")}
                    </div>
                }
            </div>
        </>
